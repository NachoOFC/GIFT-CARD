...
export default function EditGiftCardModal({ open, onClose, card, onSave }) {
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      valor_inicial: form.valor_inicial === '' ? undefined : Number(form.valor_inicial),
      ...
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg z-10">
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-3">
            ...
            <textarea value={form.mensaje} onChange={e => handleChange('mensaje', e.target.value)} className="w-full border rounded px-3 py-2" rows={3} />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="mb-3">
              <label className="block text-sm text-gray-700 mb-1">Email</label>
              <input type="email" value={form.email_destinatario} onChange={e => handleChange('email_destinatario', e.target.value)} className="w-full border rounded px-3 py-2" />
            </div>
            <div className="mb-3">
              <label className="block text-sm text-gray-700 mb-1">Valor inicial</label>
              <input type="number" value={form.valor_inicial} onChange={e => handleChange('valor_inicial', e.target.value)} className="w-full border rounded px-3 py-2" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 items-center">
            <div className="mb-3">
              ...
              <input type="date" value={form.fecha_expiracion} onChange={e => handleChange('fecha_expiracion', e.target.value)} className="w-full border rounded px-3 py-2" />
            </div>
            <div className="flex items-center space-x-2">
              <input id="activa" type="checkbox" checked={form.activa} onChange={e => handleChange('activa', e.target.checked)} />
