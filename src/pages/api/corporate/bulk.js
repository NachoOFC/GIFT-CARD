...
export default async function handler(req, res) {
  ...
  const { empresa, cliente, clientOrderId, items, notificaciones } = req.body || {};

  if (!empresa) {
    return res.status(400).json({ success: false, message: 'empresa es requerida' });
  }
  if (!Array.isArray(items) || items.length === 0) {
    return res.status(400).json({ success: false, message: 'items debe ser un arreglo con al menos un elemento' });
  }
  ...
  if (invalid.length > 0) {
    return res.status(400).json({ success: false, message: 'items con monto inválido', invalid: invalid.map(i => ({ index: i.index, valor_inicial: i.valor_inicial })) });
  }
  ...
  try {
    ...
    if (clientOrderId) {
      ...
      if (existingOrders.length > 0) {
        ...
        return res.status(409).json({ success: false, message: 'clientOrderId ya existe', order_id: existingOrders[0].id });
      }
    }
    ...
    if (itemsToInsert.length === 0) {
      await connection.rollback();
      return res.status(200).json({ success: true, inserted: 0, skipped: conflicts.length, conflicts, results: [] });
    }
    ...
    return res.status(201).json({ success: true, order: { id: orderId, numero_orden: numeroOrden, empresa, total, cantidad: results.length, estado: 'pendiente' }, inserted: results.length, skipped: conflicts.length, conflicts, results });
  } catch (error) {
    await connection.rollback();
    console.error('Error en corporate bulk:', error);
    return res.status(500).json({ success: false, message: 'Error al procesar orden corporativa', detail: String(error && (error.sqlMessage || error.message || error)) });
  } finally {
    connection.release();
  }
}
